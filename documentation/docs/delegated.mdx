...
frequency of the payments, and the discovery protocol provides seamless way for
account owners to share their account details with the 3rd party.
...
This will initiate an OAuth consent flow for the account owner with its wallet.
...
invoices `/charges` endpoint along with the `access_token` it received from the
authorization flow.
