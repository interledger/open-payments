...
The `application/json` representation of a connection resource provides
connection credentials for establishing a STREAM connection.
...
The connections API endpoints are relative to the account or invoice URL that is
being paid using the sub-resource URL: **`./connections`**

Specifically, the URL of the connections API endpoint is the result of
performing the URL parsing algorithm in
[WHATWG URL](https://url.spec.whatwg.org/#url-parsing) where:

- _input_ is `./connections`, and
- _base_ is the account or invoice URL

For example, for the account URL `https://wallet.example/alice` the connections
API endpoint for that account is `https://wallet.example/alice/connections`
...
account or invoice resource using the sub-resource path `/connections`.
