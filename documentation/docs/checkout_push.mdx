...
Receiver->>Acquirer: POST /receiver
activate Receiver
activate Receiver
activate Receiver
activate Acquirer
...
The merchant creates the invoice by making a POST request to the `/merchant` path with the `Content-Type` header set to `application/invoice+json`.
