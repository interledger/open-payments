...
the Issuer would create an invoice on the Acquirer by making a POST request with the 
`Content-Type` header set to `application/invoice+json` and a JSON payload such as the following:
