...
Receiver->>Issuer: POST /sender <br/>application/mandate+json
activate Receiver
activate Issuer
...
Receiver->>Acquirer: POST /receiver<br/>application/invoice+json
activate Receiver
activate Acquirer
...
deactivate Acquirer
deactivate Receiver
Receiver->>Issuer: POST /mandates/{id}<br/>application/invoice+json
activate Receiver
activate Issuer
...
The Merchant then creates a mandate on the Issuer wallet by making a POST request with the `Content-Type` header
set to `application/mandate+json` like so:
