...
PISP->>Issuer: POST /sender<br/>application/mandate+json
activate PISP
activate Issuer
...
alt Send Payment
    ...
    PISP->>Acquirer: POST /receiver<br/>application/invoice+json
    activate Acquirer
    activate PISP
...
PISP->>Issuer: POST /mandates/{id}<br/>application/invoice+json
activate PISP
activate Issuer
...
Content-Type: application/mandate+json
...
```http
POST /mandates/2fad69d0-7997-4543-8346-69b418c479a6 HTTP/1.1
Host: issuer.wallet
Authorization: Bearer random_auth_token
Accept: application/json
Content-Type: application/invoice+json
