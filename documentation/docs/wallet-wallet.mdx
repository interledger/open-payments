...
against the invoice URI per [Invoice Payment Details](invoices#payment-details). The request will return a 
...
When the Sender does a query for [Invoice Payment Details](invoices#payment-details) by doing an HTTP OPTIONS against
