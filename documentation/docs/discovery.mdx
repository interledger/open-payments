...
At the heart of all interactions in Open Payments is an account. The account
could be either the sending or the receiving side of the transaction. The entity
initiating the transaction is the Open Payments client and discovers the account
at the Open Payments server.

In the following example of a person-to-person payment from Alice to Bob,
Alice's wallet is the client and therefor her wallet discovers Bob's account at
his wallet to begin the transaction.
...
accounts which are at the root of the resource interaction that make up an Open
...
<Mermaid
  chart={`erDiagram
  OWNER ||--|{ ACCOUNT : has
  URL }|--|| ACCOUNT : identifies
  URL ||--|| PAYMENT-POINTER : "encoded as"
`}
/>
