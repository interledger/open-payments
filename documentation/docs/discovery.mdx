...
Account Servicing entities that support Open Payments will define one or more
unique Payment Pointer URLs for each account they service on behalf of the
owner.
...
To access the document client MUST make an HTTP GET request to the Payment
...
`application/json` media-type) and receives and error this suggests that the
