...
Given a receiver's Payment Pointer, a Client creates an Invoice by doing an HTTP POST of a JSON document to the 
HTTP URL resolved from the Payment Pointer, with the `Content-Type` header equal to `application/invoice+json`.
