...
:::note
The sequence diagrams on this page show interactions between all participants in the Open Payments flow, including both ASEs' servers and the client. For implementation details on the ASE-side components, refer to the [ASE implementation guide](/implement/).
:::
