...
:::note[For ASEs]
The full sequence diagram below shows interactions between all participants, including both the sender's and recipient's ASEs. For details on implementing the ASE-side components shown in this diagram, refer to the [Implement Open Payments](/implement/) section.
:::
