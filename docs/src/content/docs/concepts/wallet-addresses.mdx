...
Your wallet address server must enforce these URL constraints and serve the JSON response described above for every valid wallet address. Each field in the response (`id`, `publicName`, `assetCode`, `assetScale`, `authServer`, `resourceServer`) must be included in the response, and you handle currency conversion when payments arrive in a different asset. Refer to the [ASE implementation guide](/implement/) for the broader server-side context.
:::
...
You'll obtain wallet addresses from your users during onboarding and use `GET` requests to retrieve account details. You don't need to implement the wallet address server yourself. The user's ASE operates it.
:::
...
:::note[If you're an ASE]
You must decide how to map wallet addresses to underlying accounts. The models described here (1:1, 1:many, many:1) represent policy choices your institution must make.
:::
