...
You must operate an authorization server that processes grant requests, issues access tokens, and validates tokens when your resource server receives API calls. For outgoing payment grants, your auth server must initiate an interactive flow and integrate with your identity provider.
:::

:::note[If you're a developer]
As a client developer, you request grants from the ASE's authorization server and use the returned access tokens to call the resource server APIs. For outgoing payment grants, you must handle the interactive redirect flow.
:::
