...
## Receive and debit amounts

A receive(d) amount is the amount that was or will be paid into the recipient's account in the recipient's asset/currency.

A debit amount can represent one of two amounts. The first is the total amount that was or will be debited (withdrawn) from a sender's account in the sender's asset/currency. For example, in the guide for [fixed-amount recurring payments](/guides/recurring-subscription-incoming-amount), the debit amount is the amount that will be debited from the customer's payment account each month.

A debit amount could also be the maximum amount that can be debited from a sender's account in the sender's asset/currency. For example, in the guide for [getting an outgoing payment grant for future payments](/guides/outgoing-grant-future-payments), the debit amount is the maximum amount that _could_ be debited from a sender's account over one or more payments in a specific interval.
