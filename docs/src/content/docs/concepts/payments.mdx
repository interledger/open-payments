...
:::tip[Summary]
A payment method tells the sender's ASE how to deliver funds to the recipient's ASE. The recipient's ASE specifies the payment method in the `incoming-payment` response. Interledger (ILP) is currently the only payment method integrated with Open Payments.
:::
