...
When your resource server receives a Create Incoming Payment request, it must generate unique payment details (such as a wallet address) and return them in the response. Your server determines expiration behavior and how to handle incoming payments with and without an `incomingAmount`.
:::

:::note[If you're a developer]
As a developer, you create incoming payment resources by calling the API. You'll use the returned payment details in subsequent steps. You don't generate wallet addresses yourself.
:::
...
:::note[If you're an ASE]
Your resource server must calculate fees, determine exchange rates if applicable, and set a validity window for the quote. The quote represents your commitment to deliver a specific amount to the recipient's ASE.
:::
...
:::note[If you're an ASE]
After the outgoing payment resource is created, your institution is responsible for executing and settling the payment with the recipient's ASE over a shared payment rail. Open Payments provides the instruction, but you handle execution.
:::

:::note[If you're a client developer]
As a client developer, creating the outgoing payment resource is the final step in the Open Payments flow. Settlement happens between the ASEs outside Open Payments.
:::

## Grants

For information about the grant types for each resource, refer to the [Grant types](/concepts/auth/#grant-types) section in the Authorization concepts page.
