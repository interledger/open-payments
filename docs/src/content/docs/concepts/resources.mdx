...
:::note[For ASEs]
This page covers Open Payments resources from the perspective of a client developer. If you're implementing Open Payments at your institution, see [Resource server](/implement/resource-server/) for the server-side responsibilities, including how to generate payment details.
:::
...
After the `outgoing-payment` resource is created, the incoming payment can complete (either automatically or manually) to end the payment flow. The sender's ASE then settles with the recipient's ASE over a shared payment rail. For details on the server-side handling of each resource type refer to [Resource server](/implement/resource-server/).
