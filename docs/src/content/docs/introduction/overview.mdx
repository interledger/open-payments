...
Suppose a merchant accepts Open Payments as a payment method. Their customers can then pay using their wallet address instead of entering a credit card number and other personal details on the merchant’s site. Open Payments allows an ASE's account holders to use their wallet addresses for one-time and recurring purchases, such as subscriptions. Refer to the [one-time payment guide](/guides/make-onetime-payment) and the [recurring payments guide](/guides/make-recurring-payments) for more details.
