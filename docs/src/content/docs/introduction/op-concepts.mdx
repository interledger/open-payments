...
Open Payments is designed to support a variety of payment methods including, but not limited to, credit cards, debit cards, mobile money, gift cards and <LinkOut href='https://interledger.org'>Pay with Interledger</LinkOut>.

:::note
Currently the open-source implementation of Open Payments called <LinkOut href='https://rafiki.dev'>Rafiki</LinkOut> only supports `ilp` (Pay with Interledger) as a payment method. Please see the <LinkOut href='https://openpayments.dev/apis/resource-server/operations/create-quote/'>Create a Quote</LinkOut> endpoint for more details.  
:::
