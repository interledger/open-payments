...
In Open Payments, a grant indicates a transfer, or delegation, of authorization from a resource owner to a piece of software. An RO can be a physical person, such as the software’s end user, or a process, such as predefined organizational rules. By delegating authorization, the resource owner allows the software to access and perform operations on protected resources on the RO’s behalf.
...
2. Send request to create an incoming payment resource to the resource server of recipient's ASE
3. Request a `quote` grant from the authorization server of sender's ASE
4. Send request to create a quote resource to the resource server of sender's ASE
5. Request an interactive `outgoing-payment` grant from the authorization server of sender's ASE
6. Send request to create an outgoing payment resource to the resource server of sender's ASE

#### Path 2

1. Request an `incoming-payment` grant from the authorization server of recipient's <Tooltip content="Account servicing entity" client: load><span>ASE</span></Tooltip>
2. Send request to create an incoming payment resource to the resource server of recipient's ASE
3. Request a single interactive grant for both `quote` and `outgoing-payment` from the authorization server of sender's ASE
4. Send request to create a quote resource to the resource server of sender's ASE
5. Send request to create an outgoing payment resource to the resource server of sender's ASE

## Grant types

### incoming-payment

A client begins the Open Payments flow by requesting an incoming payment grant from the authorization server on the _recipient_ side. The client can request a single grant to create multiple incoming payments for different Open Payments-enabled accounts as long as each account belongs to the same <Tooltip content="Account servicing entity" client: load><span>ASE</span></Tooltip>.
