...
The goal is to address the following aspects of message security:
...
Various digital signature algorithms can be used to generate HTTP message signatures. 
The Open Payments API implements the **Ed25519** variant of the EdDSA (Edwards-curve Digital Signature Algorithm), an elliptic curve cryptographic algorithm.

 Advantages for using EdDSA over previous generations of public key cryptography algorithms include:
...
## Signature creation
Below is an example of creating the signature, starting with the original HTTP message.
...
The signature creation process begins with identifying the fields of the original message to use use when creating the signature. 
...
{/* 
    Note: contents of codeblock example below were generated using: https://httpsig.org
 */}
...
- The signature base gets hashed (using SHA-512), producing a digest.
- The digest gets signed with the signer's private key, producing the signature as a byte string.   
- The byte string gets Base64 encoded, and this results in the final signature value.

![Create the http signature](/img/http-sig/4-sig_creation.png)
...
:::note
HTTP messages can have multiple signatures, with each signature uniquely labelled.
If required, different signatures can be generated using different signature algorithms.
:::
