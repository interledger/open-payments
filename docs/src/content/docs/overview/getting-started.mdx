...
Buy Now and Pay Later (BNPL) plans allow customers to pay for high-priced purchases over time. A customer is relieved from making a large payment at once by having their payments split up at scheduled intervals. Developers can build Open Payments applications that support BNPL plans through recurring payments.
