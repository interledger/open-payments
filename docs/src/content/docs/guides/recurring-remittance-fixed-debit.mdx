...
Use the access token returned in the outgoing payment grant continuation response (Step 7) to call the <Badge text="POST" variant="success" /> [Create Outgoing Payment API](/apis/resource-server/#tag/outgoing-payment/POST/outgoing-payments). Create this payment by referencing the new `incomingPayment` URL and the fixed `debitAmount`.
