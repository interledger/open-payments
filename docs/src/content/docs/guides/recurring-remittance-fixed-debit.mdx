...
At the next interval (one month from now), create a new incoming payment (Step 3), then create another outgoing payment (Step 8) using the existing outgoing and incoming payment grants as long as they remain valid. You do not need to request new grants at each interval.

If either request fails because the grant's access token has expired, call the <Badge text="POST" variant="success" /> [Rotate Access Token API](/apis/auth-server/operations/post-token/) to obtain a new token, then retry the request with the new token.
