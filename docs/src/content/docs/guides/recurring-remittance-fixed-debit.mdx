...
The first payment is now set up. You can optionally complete the incoming payment below.

At the next interval (one month from now), create a new incoming payment (Step 3), then create another outgoing payment (Step 8) using the existing outgoing payment grant from Step 7. You do not need to request a new outgoing payment grant for each interval as long as the current grant remains valid.

If the request fails because the grant's access token has expired, call the <Badge text="POST" variant="success" /> [Rotate Access Token API](/apis/auth-server/operations/post-token/) to obtain a new token, then retry the request with the new token.
...
First, call the <Badge text="GET" variant="note" /> [Get an Outgoing Payment API](/apis/resource-server/operations/get-outgoing-payment/) to check the outgoing payment's status. Consider the payment complete when `sentAmount` equals `debitAmount` and `failed` is false.
