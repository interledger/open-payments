...
:::tip[Summary]
Learn how an Open Payments-enabled client can be used to set up a payment of a specific amount that pays out at recurring intervals.
:::

The Open Payments APIs facilitate multiple use cases for recurring payments to and from Open Payments-enabled wallets. Paying a fee for a monthly subscription is one example.

This guide provides the steps your Open Payments-enabled client will take to set up a $50 USD payment for a user that recurs once a month for three months.
...
The `value` is the maximum allowable amount that can be paid into the resource. Since we're setting up a $50 USD payment that recurs once a month for three months, the `value` is `15000` ($150).
...
The sender indicates they want to make payments of $50 a month for three months. The amount resets each month and any unspent portions don't roll over.
...
The interval used in this guide is R3/2025-05-20T13:00:00Z/P1M. Remember that your user wants to send payments of $50 USD a month for three months. The interval breaks down like this:
...
Recall that the Create Quote API (Step 5) included a `debitAmount` and a `receiveAmount`. The response also included an `id` which is a URL to identify the quote.
...
### 11. Rotate the access token (conditional)

:::note
This step is only required if the original outgoing payment grant's access token has expired.
:::
...
When the time comes for the next recurring payment to be made, request the creation of another outgoing payment resource
