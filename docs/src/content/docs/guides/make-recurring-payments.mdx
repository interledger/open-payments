...
Request an `incomingPayment` grant from the recipient wallet's AS.
...
Create an `incomingPayment` on the recipient wallet's Resource Server (RS) using the access token returned by the AS in the grant request and specify the `walletAddress` with the URL of the recipient's wallet.
...
Request a `quote` grant from the sender wallet's AS.
...
Create a `quote` on the sender wallet's RS using the access token returned by the AS in the grant request.
...
Request an `outgoingPayment` grant from the sender wallet's AS.
...
:::note
This request requires an interactive grant as the user will need to consent before an `outgoingPayment` is made against their wallet.
...
Create an `outgoingPayment` on the sender wallet's RS using the access token returned by the AS in the grant request.
...
Create another `outgoingPayment` on the sender wallet's RS using the new access token returned by the AS in the previous step.
