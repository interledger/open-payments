...
```json wrap
{
  "id": "https://happylifebank.example.com/incoming-payments/{...}", // url identifying the incoming payment
  ...
  "methods": [
    {
      "type": "ilp",
      "ilpAddress": "...", // ilp address to use when establishing a STREAM connection
      "sharedSecret": "..." // base64 url-encoded shared secret to use when establishing a STREAM connection
    }
  ]
}
```
...
This call requests a quote resource be created on the customer's wallet account. Since the customer needs a quote resource for the merchant and the platform, we'll call the API twice using the same access token.
  ...
  const pendingCustomerOutgoingPaymentGrant = await client.grant.request(
    ...
    },
    {
      ...
      interact: {
        ...
        finish: {
          method: 'redirect',
          uri: 'https://paymentplatform.example/finish/{...}', // finish uri of the customer wallet address' auth server
          nonce: NONCE
        }
      }
    }
...
Call the [Grant Continuation Request API](/apis/auth-server/operations/post-continue). This call requests an access token that allows your platform to request outgoing payment resources be created on the customer's wallet account.
...
{
  "access_token": {
    "value": "...", // access token value for the outgoing payment grant continuation
