...
<Card title="Example">
  The merchant charges a flat $2.00 for shipping. The response associated with the merchant's incoming payment resource would have a `debitAmount` of `10100` ($99 + $2). 
  
  Your platform doesn't charge the customer any fees, so the `debitAmount` and the `receiveAmount` are the same (`100`).

With this information, your platform can display a message to the customer that states their total payment amount will be $102.00 USD.
