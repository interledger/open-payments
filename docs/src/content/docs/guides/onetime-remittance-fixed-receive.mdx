...
Imagine someone in the US wants to send money to a family member in Mexico. They want their family member to receive _exactly_ $5,000 Mexican pesos (MXN), even if it means paying a slightly higher amount in US dollars to cover currency conversion. This is different from a payment where the sender specifies exactly how much to send, regardless of what the recipient will receive.
...
- `debitAmount`: taken from the sender's account, denominated in the sender's currency (in our example, USD)
- `receiveAmount`: delivered to the recipient's account, denominated in the recipient's currency (in our example, MXN)
- See [Amounts](/concepts/amounts/) for more information about asset codes, asset scales, and values.
...
Quotes may include an `expiresAt` timestamp. Create the outgoing payment before the quote expires. If creation fails because the quote expired, request a new quote and try again.
:::
...
If you want to complete the incoming payment and surface a payment confirmation in your app, then call the <Badge text="POST" variant="success" /> [Complete Incoming Payment API](/apis/resource-server/operations/complete-incoming-payment/).
:::
