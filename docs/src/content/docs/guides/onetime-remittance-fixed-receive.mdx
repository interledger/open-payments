...
- The sender is willing to cover the cost of any fees and changes in exchange rate

## Scenario

Imagine someone in the US wants to send money to a family member in Mexico. They want their family member to receive _exactly_ $5,000 Mexican pesos (MXN), even if it means paying a slightly higher amount in US dollars to cover fees and currency conversion. This is different from a payment where the sender specifies exactly how much to send, regardless of what the recipient will receive.

For this guide, you'll assume the role of a developer building a remittance app. The guide explains how to send a payment in USD, where the recipient receives exactly MXN $5,000.

**Example transaction details:**

- **Recipient receives**: MXN $5,000 (exact amount)
...
- **Sender pays**: Amount in USD required to cover MXN $5,000 plus fees
...
Remember, Open Payments doesn't execute payments or touch money in any way. It's used to issue payment instructions before any money movement occurs. An example of a payment instruction is, "Debit an amount in USD from the sender's account that's sufficient to ensure the recipient receives exactly MXN $5,000 after all fees and USD to MXN conversion."
...
The `receiveAmount` specifies that the recipient will receive exactly MXN $5,000.
...
- `receiveAmount` - The amount the recipient will actually receive (exactly MXN $5,000 in our example).
...
```json wrap
{
  ...
  "receiveAmount": {
    "value": "500000", // Recipient receives MXN $5,000
    "assetCode": "MXN",
    "assetScale": 2
  },
  "method": "ilp",
...
To ensure the sender is authorizing the correct amount, the `receiveAmount` is included in the `limits` object. This limits the outgoing payment to the specified receive amount. The `receiveAmount` is already defined in the quote resource that will be used when requesting the creation of an outgoing payment resource (Step 10).
