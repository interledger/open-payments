...
This guide explains how to implement Open Payments in your remittance app so that the recipient receives the specified amount (MXN $5,000 in this example), even if the sender has to pay a slightly higher amount in US Dollars to cover fees and currency conversions.

**Example transaction details:**

- **Recipient receives**: MXN $5,000.00 (exact amount)
...
The `receiveAmount` specifies that the recipient will receive exactly MXN $5,000.00, and the sender will pay whatever amount is required after all fees and currency conversion.
...
- `receiveAmount` - The amount the recipient will actually receive (exactly MXN $5,000.00 in our example).
...
```json wrap
{
  ...
  "receiveAmount": {
    "value": "500000", // Recipient receives MXN $5,000.00
    "assetCode": "MXN",
    "assetScale": 2
  },
  "method": "ilp",
