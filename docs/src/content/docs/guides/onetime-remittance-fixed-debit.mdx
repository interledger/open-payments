...
Call the <Badge text="POST" variant="success" /> [Grant Continuation Request API](/apis/auth-server/#tag/grant/POST/continue/%7Bid%7D). This call requests an access token that allows your app to request that an outgoing payment resource be created on the sender's wallet account.
