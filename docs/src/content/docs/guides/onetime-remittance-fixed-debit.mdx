...
- **Recipient receives**: MXN $1,615.00 (($100 - $5) × 17.00)
...
Remember, Open Payments doesn't execute payments or touch money in any way. It's used to issue payment instructions before any money movement occurs. An example of a payment instruction is, "debit exactly $100 from the sender's account and keep $5 in fees from the $100 debited from the sender's account".
...
```json wrap
{
  "id": "https://happylifebank.example.com/sender",
  "assetCode": "MXN",
  "assetScale": 2,
...
- `receiveAmount` - The amount the recipient will actually receive (MXN $1,650.00 in our example) after fees and currency conversion.
...
```json wrap
{
  ...
  "receiveAmount": {
    "value": "161500", // Recipient receives MXN 1,615.00  after fees and currency conversion
    "assetCode": "MXN",
    "assetScale": 2
  },
  "method": "ilp",
...
Use the access token returned in Step 9 to call the Create Outgoing Payment API. Include the quoteId in the request. The quoteId is the id returned in the Create Quote API response (Step 5).
