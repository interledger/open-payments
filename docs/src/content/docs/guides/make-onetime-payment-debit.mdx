...
:::tip[Summary]
Learn how to send a direct payment where the sender specifies a fixed amount to send (debitAmount) rather than the recipient receiving a fixed amount.
:::

Imagine you're sending money to a family member overseas. You want to send exactly $100 USD from your account, regardless of how much they actually receive after fees and exchange rates. This is different from sending money where you specify how much the recipient should receive - in that case, you'd pay whatever fees are necessary to ensure they get the exact amount.

With Open Payments, you can create payment instructions that specify exactly how much should be debited from your account (the `debitAmount`), while the recipient receives whatever amount remains after fees and currency conversion.
...
For this guide, you'll assume the role of a sender making a direct payment. The guide explains how to send a $100 USD payment where you pay exactly $100 (plus any fees), and the recipient receives the amount after all fees and currency conversion.

The two parties involved in the transaction are the:

- Sender: you, the person sending money
...
When you initiate a payment, you need to get wallet address information for both yourself (the sender) and the recipient.

Let's assume you have the recipient's wallet address. You'll also need your own wallet address information.
...
This call obtains an access token that allows you to request an incoming payment resource be created on the recipient's wallet account.
...
This call requests an incoming payment resource be created on the recipient's wallet account. Note that we do NOT specify an `incomingAmount` - this allows the recipient to receive whatever amount remains after fees and currency conversion.
...
Use your `authServer` details, received in Step 1, to call the [Grant Request API](/apis/auth-server/operations/post-request).

This call obtains an access token that allows you to request quote resources be created on your wallet account.
...
This call requests a quote resource be created on your wallet account. The request must contain the `receiver`, which is the recipient's incoming payment `id`, along with the `debitAmount` - the exact amount you want to pay.

The `debitAmount` specifies that you will pay exactly $100 USD (plus any fees), and the recipient will receive whatever amount remains after all fees and currency conversion.
...
Use your `authServer` information received in Step 1 to call the [Grant Request API](/apis/auth-server/operations/post-request).

This call obtains an access token that allows you to request outgoing payment resources be created on your wallet account.
...
In our example, we're assuming the IdP you interacted with has a user interface. When the interaction completes, you return to your application. Now your application can make a continuation request for the outgoing payment grant.
...
Call the [Grant Continuation Request API](/apis/auth-server/operations/post-continue). This call requests an access token that allows you to request outgoing payment resources be created on your wallet account.
...
In this guide, you learned how to create a one-time payment where you specify exactly how much you want to pay (`debitAmount`), rather than how much the recipient should receive. This is particularly useful for direct payment scenarios where:

- You want to pay a fixed amount from your account
- The recipient receives whatever amount remains after fees and currency conversion
- You want to avoid the complexity of calculating fees upfront

The key difference from other payment flows is that the incoming payment resource is created without specifying an `incomingAmount`, and the quote is created with a `debitAmount` that specifies exactly how much you will pay.
