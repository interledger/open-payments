...
Use the sender wallet's `authServer` details, received in Step 1, to call the [Grant Request API](/apis/auth-server/operations/post-request).
...
Use the sender wallet's `authServer` information, received in Step 1, to call the [Grant Request API](/apis/auth-server/operations/post-request).
...
Call the [Grant Continuation Request API](/apis/auth-server/operations/post-continue). This call requests an access token that allows your client to request an outgoing payment resource be created on the sender's wallet account.
