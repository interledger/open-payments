...
- `assetCode` : the ISO 4217 currency code representing the underlying asset used to make the payment. As an example, the US Dollar is represented as USD.
- `assetScale` : the number of decimal places used to represent the minimum value in the corresponding asset code. USD has an asset scale of 2 with a minimum value of $0.01. 
