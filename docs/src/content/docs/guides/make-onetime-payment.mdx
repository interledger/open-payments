...
This guide provides the steps an Open Payments-enabled client will take to set up a one-time $10 USD payment from you, the sender, to a recipient.
...
When you initiate a payment, your client must get wallet address information for both you, as the sender, and the recipient.

Let's assume your wallet address is already saved in your client. Let's also assume that you've entered the recipient's wallet address into a payment form in your client.
...
The following shows an example response from the recipient's wallet provider. A similar response will be returned from your wallet provider.
```json wrap
{
...
Use your wallet's `authServer` details, received in Step 1, to call the [Grant Request API](/apis/auth-server/operations/post-request).

This call obtains an access token that allows your client to request a quote resource be created on your wallet account.
...
This call requests a quote resource be created on your wallet account. The request must contain the `receiver`, which is the incoming payment `id` returned from the recipient's wallet in Step 3.
...
Use your wallet's `authServer` information, received in Step 1, to call the [Grant Request API](/apis/auth-server/operations/post-request).

This call obtains an access token that allows your client to request an outgoing payment resource be created on your wallet account.
  ...
  const pendingSenderOutgoingPaymentGrant = await client.grant.request(
    ...
    },
    {
      ...
      interact: {
        ...
        finish: {
          method: 'redirect',
          uri: 'https://paymentplatform.example/finish/{...}', // where your client will redirect you to after you've completed the interaction
          nonce: NONCE
        }
      }
    }
...
{
  "interact": {
    "redirect": "https://auth.interledger-test.dev/{...}", // uri the client will redirect you to, to begin the interaction
...
### 7. Start interaction with the user (you, as the sender)

<StartInteraction />

### 8. Finish interaction with the user (you, as the sender)

<FinishInteraction />

### 9. Request a grant continuation

In our example, we're assuming the IdP you interacted with has a user interface. When the interaction completes, you are returned to your client. Now your client can make a continuation request for the outgoing payment grant.
...
Call the [Grant Continuation Request API](/apis/auth-server/operations/post-continue). This call requests an access token that allows your client to request an outgoing payment resource be created on your wallet account.
...
Use the access token returned in Step 6 to call the [Create Outgoing Payment API](/apis/resource-server/operations/create-outgoing-payment). Include the `quoteId` in the request.
