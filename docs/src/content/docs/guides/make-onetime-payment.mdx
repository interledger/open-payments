...
- `assetCode` : the ISO 4217 currency code representing the underlying asset used to make the payment. As an example, the euro is represented as EUR.
- `assetScale` : the number of decimal places used to represent the minimum possible value in the corresponding asset code. EUR has an asset scale of 2 with a minimum possible value of €0.01.
