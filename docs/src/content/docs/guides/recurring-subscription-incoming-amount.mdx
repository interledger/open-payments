...
The `incomingAmount` specifies the exact amount the service provider wants to receive for the subscription. Since the monthly subscription is \$15 USD for 12 months, the total subscription amount is \$180 USD. The `value` is `18000`.
...
This call requests that a quote resource be created on the customer's wallet account. The request must contain the `receiver`, which is the service provider's incoming payment `id`, along with the `incomingAmount`, which is the exact amount the service provider wants to receive.

The `incomingAmount` specifies that the service provider will receive exactly \$15 USD.
...
- `receiveAmount` - The amount the service provider will receive.
...
The first of the twelve recurring subscription payments is now set up. At the next interval (one month from now), repeat Steps 5 and 10. You don't need to request new incoming payment, quote, or outgoing payment grants (Steps 2, 4, and 6) because the original grants are still valid for the remaining billing periods.

:::note[Access token expiry]
If a particular grant's access token has expired, call the <Badge text="POST" variant="success" /> [Rotate Access Token API](/apis/auth-server/operations/post-token), then use the new token in the appropriate incoming payment request, quote request, or outgoing payment request.
:::
