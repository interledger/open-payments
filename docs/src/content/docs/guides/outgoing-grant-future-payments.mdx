...
Use the authorization server information received in the previous step to call the [Grant Request API](/apis/auth-server/operations/post-request).
...
Call the [Grant Continuation Request API](/apis/auth-server/operations/post-continue/). This call requests an access token that allows your app to request outgoing payment resources be created on the user's wallet account.
