...
For clients that identify themselves with a [wallet address](/concepts/wallet-addresses/), the ASE provides a way for clients to register public keys and hosts the client's key registry at `WALLET_ADDRESS/jwks.json`. An authorization server can retrieve the client's key registry from that endpoint.
