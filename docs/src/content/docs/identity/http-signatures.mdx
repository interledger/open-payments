...
Signature creation starts with the original HTTP message.
...
### Signature base and signature params

First, the covered components are identified. The covered components are the fields that identify which parts of the message to use when creating the signature.

The signature base is comprised of the covered components, the signing algorithm, and an identifier for the signer's public key.
...
This is what you implement: sign your outgoing requests using your private key, including the `Signature-Input` and `Signature` headers as described above.
:::

:::note[If you're an ASE]
This is what you implement: validate incoming request signatures using the client's public key (resolved via the wallet address `jwks.json` endpoint or supplied directly via directed identity). Refer to [Security](/implement/security/) for the server-side validation procedure.
:::
