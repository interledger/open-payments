...
These are the steps your client follows:

1. Build a signed grant request that names the resource type and actions.
2. Send it to the authorization server.
3. Use the returned access token on subsequent API calls.
   :::
...
Your authorization server must implement the GNAP endpoints described on this page and handle the request/response patterns shown. This includes signature validation, key resolution, grant creation, and (for interactive grants) issuing continuation and interaction URIs. Your resource server must validate access tokens with your authorization server before serving requests. Refer to the [ASE implementation guide](/implement/) for the broader server-side context.
:::
