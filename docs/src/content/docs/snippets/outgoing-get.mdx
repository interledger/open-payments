...
An authorized client can look up the state of an outgoing payment on a payment pointer. This is useful when a client must determine if an outgoing payment is still active and pending payment.
...
<Disclosure toggleText='View full source' client:load>
  <FullSnippet source='https://raw.githubusercontent.com/interledger/open-payments-snippets/main/outgoing-payment/outgoing-payment-get.ts' />
</Disclosure>
