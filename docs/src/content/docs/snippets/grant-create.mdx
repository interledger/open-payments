...
Open Payments also requires any authorization server that issues interactive grants to integrate with an [identity provider (IdP)](/introduction/idp). When the client requests the outgoing payment grant, the authorization server provides the client with the IdP URI the client should redirect to.
