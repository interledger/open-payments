...
Each client is represented by a [wallet address](/1.0/concepts/wallet-addresses). A client's key registry is publicly accessible through its wallet address via a `jwks.json` endpoint. An authorization server can retrieve the client's key registry by accessing `WALLET_ADDRESS/jwks.json`.
...
See the [HTTP message signatures](/1.0/identity/http-signatures) page for more information specific to Open Payments. Additional information is found in the <LinkOut href="https://datatracker.ietf.org/doc/html/rfc9421">specification</LinkOut> for HTTP message signatures.
