...
Utilice los datos del minorista `authServer`, recibidos en el paso anterior, para llamar a la <Badge text="POST" variant="success" /> [Grant Request API](/apis/#tag/grant/POST/).
...
Use el token de acceso devuelto en la respuesta anterior para llamar a la <Badge text="POST" variant="success" /> [Create an Incoming Payment API](/apis/#tag/incoming-payment/POST/incoming-payments).
...
Utilice los datos del consumidor `authServer`, devueltos en el paso 1, para llamar a la <Badge text="POST" variant="success" /> [Grant Request API](/apis/#tag/grant/POST/).
...
Utilice la información del consumidor `authServer` para llamar a la <Badge text="POST" variant="success" /> [Grant Request](/apis/#tag/grant/POST/).
...
Llame a la <Badge text="POST" variant="success" /> [Grant Continuation Request API](/apis/#tag/grant/POST/continue/%7Bid%7D). Esta llamada solicita un token de acceso que permite a la aplicación cliente solicitar la creación de un recurso de pago saliente en la cuenta de billetera del cliente.
...
Utilice el token de acceso devuelto en el paso 9 para llamar a la <Badge text="POST" variant="success" /> [Create Outgoing Payment API](/apis/#tag/outgoing-payment/POST/outgoing-payments). Esta llamada solicita la creación de un recurso de pago saliente en la cuenta de billetera del consumidor. Incluya el `quoteId` en la solicitud. La `quoteId` es la `id` devuelta en la respuesta de Create Quote API (paso 5).
