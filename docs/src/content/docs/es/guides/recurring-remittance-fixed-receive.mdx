...
Utilice la información del remitente `authServer` recibida en el paso anterior para llamar a la <Badge text="POST" variant="success" /> [Grant Request API](/apis/#tag/grant/POST/).
...
Llame a la <Badge text="POST" variant="success" /> [Grant Continuation Request API](/apis/#tag/grant/POST/continue/%7Bid%7D). Esta llamada obtiene un token de acceso que permite a su aplicación continuar con la solicitud de concesión de autorización de pago saliente.
...
Utilice los datos del destinatario `authServer` recibidos en el paso 1 para llamar a la <Badge text="POST" variant="success" /> [Grant Request API](/apis/#tag/grant/POST/).
...
Utilice el token de acceso devuelto en la continuación de la concesión de autorización de pago saliente (paso 5) para llamar a la <Badge text="POST" variant="success" /> [Create Outgoing Payment API](/apis/#tag/outgoing-payment/POST/outgoing-payments).
...
Si el token de acceso de una concesión ha expirado, llame a la <Badge text="POST" variant="success" /> [Rotate Access Token API](/apis/#tag/token/POST/token/%7Bid%7D) y, luego, utilice el nuevo token en la solicitud correspondiente.
:::
