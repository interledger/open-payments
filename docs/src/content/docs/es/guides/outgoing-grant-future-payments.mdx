...
Supongamos que su usuario guardó su dirección de billetera en el perfil de su cuenta al configurar la aplicación. Llame a la [Get Wallet Address API](/apis/wallet-address-server/#tag/wallet-address/GET/) para obtener los datos necesarios sobre la cuenta de la billetera del usuario.
...
Utilice la información del servidor de autorización recibida en el paso anterior para llamar a la [Grant Request API](/apis/auth-server/#tag/grant/POST/). El propósito de esta llamada consiste en obtener un token que permita que el cliente solicite la creación de recursos de pago saliente en la cuenta de la billetera de su usuario.
