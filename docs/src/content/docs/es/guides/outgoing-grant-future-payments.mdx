...
<TabItem label="Rust" icon="seti:rust">
  ```rust wrap let user_wallet_address =
  client.wallet_address().get("https://cloudninebank.example.com/user").await?;
  ```
</TabItem>
{/* prettier-ignore-end */}
...
Si una solicitud de pago saliente falla porque el token de acceso de la concesión de autorización ha expirado, puede llamar a la <Badge text="POST" variant="success" /> [Rotate Access Token API](/apis/auth-server/operations/post-token/) y, luego, utilizar el nuevo token en la solicitud de pago saliente.
:::
