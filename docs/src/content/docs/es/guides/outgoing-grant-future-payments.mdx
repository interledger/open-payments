...
- <Badge text="GET" variant="note" />
  <LinkOut href="https://openpayments.dev/apis/resource-server/operations/get-outgoing-payment-grant/">
    Obtener los montos gastados para la concesión de autorización para el pago
    saliente actual
  </LinkOut>
...
Supongamos que su usuario guardó su wallet address en el perfil de su cuenta al configurar la aplicación. Llame a la [Get Wallet Address API](/es/apis/wallet-address-server/operations/get-wallet-address) para obtener los datos necesarios sobre la cuenta de la billetera del usuario.
...
Puede proporcionar a su usuario información sobre cuánto le queda para gastar mediante una llamada a <Badge text="GET" variant="note" /> [API Obtener los montos gastados para la concesión de autorización para el pago saliente actual](/es/apis/auth-server/operations/get-outgoing-payment-grant/).
