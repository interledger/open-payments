...
Llame a la <Badge text="GET" variant="note" /> [Get Wallet Address API](/apis/#tag/wallet-address/GET/) para cada dirección.
...
Utilice los datos del destinatario `authServer`, recibidos en el paso anterior, para llamar a la <Badge text="POST" variant="success" /> [Grant Request API](/apis/#tag/grant/POST/).
...
Use el token de acceso devuelto en la respuesta anterior para llamar a la <Badge text="POST" variant="success" /> [Create Incoming Payment API](/apis/#tag/incoming-payment/POST/incoming-payments).
...
Utilice los datos del remitente `authServer` recibidos en el paso 1 para llamar a la <Badge text="POST" variant="success" /> [Grant Request API](/apis/#tag/grant/POST/).
...
Utilice la información del remitente `authServer` recibida en el paso 1 para llamar a la <Badge text="POST" variant="success" /> [Grant Request API](/apis/#tag/grant/POST/).
...
Llame a la <Badge text="POST" variant="success" /> [Grant Continuation Request API](/apis/#tag/grant/POST/continue/%7Bid%7D). Esta llamada solicita un token de acceso que permite a su aplicación solicitar la creación de un recurso de pago saliente en la cuenta de billetera del remitente.
...
Utilice el token de acceso devuelto en el paso 9 para llamar a la <Badge text="POST" variant="success" /> [Create Outgoing Payment API](/apis/#tag/outgoing-payment/POST/outgoing-payments). Incluya el `quoteId` en la solicitud. La `quoteId` es la `id` devuelta en la respuesta de API Crear cotización (paso 5).
...
Si el token de acceso de una concesión de autorización ha expirado, llame a la <Badge text="POST" variant="success" /> [Rotate Access Token API](/apis/#tag/token/POST/token/%7Bid%7D) y, luego, utilice el nuevo token en la solicitud correspondiente.
:::
