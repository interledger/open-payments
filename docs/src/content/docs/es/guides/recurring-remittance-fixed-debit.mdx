---
title: Enviar remesas recurrentes con un monto de débito fijo
---
...
Utilice los datos del destinatario `authServer` recibidos en el paso 1 para llamar a la <Badge text="POST" variant="success" /> [API Solicitud de concesión](/apis/auth-server/operations/post-request/).
...
- `interval` - El [intervalo de tiempo](#about-the-interval) durante el cual la concesión es válida.

:::Nota
Los pagos salientes requieren una concesión de autorización interactiva, que el remitente aprueba una sola vez. Mientras la concesión de autorización (y su token de acceso) sean válidos y estén dentro de sus límites (`interval` + `debitAmount`), su aplicación puede crear pagos salientes sin volver a- aprobar cada intervalo. Puede encontrar más información en las páginas del [flujo de Open Payments](/es/concepts/op-flow/#outgoing-payment) y [proveedores de identidad](/es/identity/idp).
:::
...
Llame a la <Badge text="POST" variant="success" /> [API Continuación de solicitud de concesión](/apis/auth-server/operations/post-continue/). Esta llamada obtiene un token de acceso que permite a su aplicación continuar con la solicitud de concesión de autorización de pago saliente.
...
Utilice el token de acceso devuelto en la continuación de la concesión de autorización de pago saliente (paso 7) para llamar a la <Badge text="POST" variant="success" /> [API Crear pago saliente](/apis/resource-server/operations/create-outgoing-payment/). Cree este pago haciendo referencia a la nueva URL `incomingPayment` y al `debitAmount` fijo.
...
2. Un recurso de pago saliente ([paso 8](#8-solicitar-la-creación-de-un-recurso-de-pago-saliente))
...
Si el token de acceso de una concesión ha expirado, llame a la <Badge text="POST" variant="success" /> [API Rotación del token de acceso](/apis/auth-server/operations/post-token/) y, luego, utilice el nuevo token en la solicitud correspondiente.
