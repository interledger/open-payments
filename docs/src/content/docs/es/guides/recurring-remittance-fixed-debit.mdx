...
Utilice el token de acceso devuelto en la continuación de la concesión de autorización de pago saliente (paso 7) para llamar a la <Badge text="POST" variant="success" /> [Create Outgoing Payment API](/apis/resource-server/operations/create-outgoing-payment/). Cree este pago haciendo referencia a la nueva URL `incomingPayment` y al `debitAmount` fijo.
