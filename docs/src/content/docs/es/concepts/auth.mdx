...
import ForAsesAuthServer from '/src/content/docs/partials/es/_for-ases-auth-server.mdx'
...
Por lo general, para iniciar el flujo de Open Payments, el cliente solicita una concesión de autorización para un pago entrante al servidor de autorización que corresponde al _destinatario_. Sin embargo, existen situaciones en las que el cliente solicita una concesión de autorización para un pago saliente primero, como en el caso de <LinkOut href='https://webmonetization.org'>Web Monetization</LinkOut>.
...
Por defecto, las concesiones de autorización para pagos entrantes no son interactivas; esto significa que no se requiere la interacción de una persona (por lo general, el usuario del cliente) para que el servidor de autorización emita un token de acceso. Si la solicitud de concesión incluye la acción `list-all`, el servidor de autorización debe requerir interacción antes de emitir un token de acceso, ya que esa acción permite al cliente enumerar los pagos entrantes que no creó. Los clientes pueden utilizar la [identidad dirigida](/es/identity/client-keys#solicitudes-de-clientes) para las solicitudes de concesión para pagos entrantes.
...
Por defecto, las concesiones de autorización para cotizaciones no son interactivas; esto significa que no se requiere la interacción de una persona (por lo general, el usuario del cliente) para que el servidor de autorización emita un token de acceso. Si la solicitud de concesión incluye la acción `list-all`, el servidor de autorización debe requerir interacción antes de emitir un token de acceso, ya que esa acción permite al cliente enumerar las cotizaciones que no creó. Los clientes también pueden utilizar la [identidad dirigida](/es/identity/client-keys#solicitudes-de-clientes) para las solicitudes de concesión para cotizaciones.
...
Luego de una interacción correcta, el cliente debe emitir una [Continuation Request](/es/apis/auth-server/operations/post-continue), para que el servidor de autorización sepa que debe emitir un token de acceso.
