...
Un recurso de `incoming-payment` suele ser el primero en crearse en un flujo de pago, a través de la <LinkOut href='/apis/resource-server/operations/create-incoming-payment'>Create Incoming Payment API</LinkOut>. El recurso se crea en la cuenta del destinatario.
...
Luego de crearse un recurso de `incoming-payment` en la cuenta del destinatario, por lo general, un recurso de `quote` se crea en la cuenta del remitente a través de la <LinkOut href="/apis/resource-server/operations/create-quote">Create a Quote API</LinkOut>.
