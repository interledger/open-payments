...
La creación de la firma comienza con el mensaje HTTP original.

```http title="Ejemplo: mensaje antes de la firma"
POST HTTP/1.1
Host: example.com
...
### Base de la firma y parámetros de la firma

En primer lugar, se identifican los componentes cubiertos. Los componentes cubiertos son los campos que identifican qué partes del mensaje se usarán para crear la firma.

La base de la firma está formada por los componentes cubiertos, el algoritmo de firma y un identificador de la clave pública del firmante.
...
```http wrap title="Ejemplo: base de la firma"
"content-type": application/json
"content-digest": sha-512=:X48E9qOokqqrvdts8nOJRJN3OWDUoyWxBf7kbu9DBPE=:
"content-length": 18
"authorization": GNAP 123454321
...
```http wrap title="Ejemplo: mensaje firmado"
POST HTTP/1.1
