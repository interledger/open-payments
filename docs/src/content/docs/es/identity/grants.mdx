...
import RafikiResourceServerNote from '/src/content/docs/partials/es/_rafiki-resource-server-note.mdx'
...
Un cliente debe generar y agregar su clave a su registro de claves antes de solicitar una concesión por primera vez. Un cliente que utiliza la identidad dirigida proporciona su clave pública directamente en la solicitud de concesión y no necesita un registro de claves. Para obtener más información sobre la generación y el registro de claves y sobre cómo funcionan los registros de claves con los servidores de autorización, consulte la página [Claves de clientes](/es/identity/client-keys).
