...
The [Get Spent Amounts for Current Outgoing Payment Grant API](/apis/#tag/outgoing-payment/GET/outgoing-payment-grant) returns the debit and receive amounts spent against the current outgoing payment grant.

When an outgoing payment [grant continuation request](/apis/#tag/grant/POST/continue/%7Bid%7D) is successful, the client receives an access token from the authorization server. The current grant is identified by this access token.
...
- [Resource server API reference](/apis/#tag/outgoing-payment/GET/outgoing-payment-grant)
