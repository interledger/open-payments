...
<ChunkedSnippet
  source="outgoing-payment/outgoing-payment-list.ts"
  l="node"
  chunk="3"
/>
...
<ChunkedSnippet
  source="outgoing-payment/outgoing-payment-list.rs"
  l="rust"
  chunk="1"
/>
...
<LinkButton href="https://github.com/interledger/open-payments-java?tab=readme-ov-file#prerequisites" target="_blank" icon="external" iconPlacement="start">Prerequisites</LinkButton>
    ...
    // 10. Retrieve the created outgoing payments (by walletAddress):
    var outgoingPaymentsFetched = this.client.getOutgoingPayments(senderWallet.getId(), grantRequest);

    // 11. Logging:
    log.info("OUTGOING_PAYMENTS: {}", outgoingPaymentsFetched.size());
    outgoingPaymentsFetched.forEach(p -> log.info("OUTGOING_PAYMENT: {}", p));
    ```
