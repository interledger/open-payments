...
The [Rotate Access Token API](/apis/#tag/token/POST/token/%7Bid%7D) lets you request a new access token from the authorization server. The new token replaces the existing token and has the same rights and properties. If, for example, an access token expires, a client can request the token be rotated.
...
- [Auth server API reference](/apis/#tag/token/POST/token/%7Bid%7D)
