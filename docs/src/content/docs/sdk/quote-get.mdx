...
The [Get a Quote API](/apis/#resource-server/tag/quote/GET/quotes/%7Bid%7D) lets you get the latest details for a quote resource. For example, its state (whether it's valid or expired), the total amount that the recipient should receive, and the total amount to be debited from the sender.
...
- [Resource server API reference](/apis/#resource-server/tag/quote/GET/quotes/%7Bid%7D)
