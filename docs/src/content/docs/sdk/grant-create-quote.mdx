...
The [Grant Request API](/apis/#tag/grant/POST/) lets you request a grant for quote, outgoing payment, and incoming payment resources.
...
The code snippets below let a client request a grant for a quote. The request to the authorization server must indicate the `quote` access type and the actions the client wants to take at the resource server.
...
// Request quote grant
var grantRequest = client.auth().grant().quote(senderWallet);
...
// Check grant state
if (grant.Interact != null) throw new Exception("Expected non-interactive grant");

// Output
Console.WriteLine(JsonConvert.SerializeObject(grant, Formatting.Indented));
```

<LinkOut href="https://github.com/interledger/open-payments-dotnet/blob/main/OpenPayments.Snippets/Services/Authenticated/QuoteService.cs#L20-L36">
  View full source
</LinkOut>
</TabItem>
</Tabs>
