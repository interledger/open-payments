...
One use case is verifying that the correct user has access to a specific wallet address. For example, a payout platform can confirm during onboarding that a user controls the wallet address they provided. Refer to the [Verify ownership of a wallet address](/guides/verify-wallet-address-ownership/) guide for an example. The `subject` field can also be used for other verification scenarios.
