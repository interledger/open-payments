...
A code representation of the underlying asset used to make a payment. The `assetCode` can be an <LinkOut href='https://en.wikipedia.org/wiki/ISO_4217'>ISO 4217</LinkOut> currency code such as EUR (euro), MXN (Mexican peso) or USD (US dollar). It can also be used to represent cryptocurrencies such as BTC (Bitcoin), ETH (Ethereum) or USDC (USD stablecoin). 
