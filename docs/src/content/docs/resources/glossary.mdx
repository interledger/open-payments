...
An `assetCode` is a code representation of the underlying asset used to make a payment. The `assetCode` should be an <LinkOut href='https://www.iso.org/iso-4217-currency-codes.html'>ISO 4217</LinkOut> currency code. The ISO 4217 currency code representing the US Dollar is USD.

## Asset scale

The `assetScale` is the number decimal places used to represent the minimum value of an asset. As an example, USD has an asset scale of 2 with a minimum value of $0.01. An interger amount of 1000 with an `assetCode` of USD and an `assetScale` of 2 translates to $10.00.
