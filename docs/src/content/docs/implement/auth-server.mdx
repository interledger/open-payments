...
The grant endpoint URI is what the wallet address response advertises as `authServer`. Clients send all grant requests there. The GNAP specification uses a single endpoint per authorization server rather than a different endpoint per grant type.
...
2. Persist the grant in a `pending` state, including the requested limits (amounts, expiry, etc.).
...
4. When the resource owner reaches the redirect URI, start the interaction session and forward the resource owner to the identity provider. Refer to [Identity provider integration](/implement/identity-provider/).
...
7. When the client posts to the continuation URI with the continuation token, issue the access token if the grant is `approved`. If the grant is still `pending` or has been denied, return the appropriate GNAP error.
