...
- Set a validity window after which the quote is no longer honored. ASEs define this window; it must be short enough that rate and fee exposure is bounded and long enough that the quote remains valid through any later steps required before execution, including interactive consent and the subsequent Create Outgoing Payment request.
