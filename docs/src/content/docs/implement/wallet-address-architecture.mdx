...
- `id` — The canonical wallet address URL. ASEs must return the same identifier the client used to reach the endpoint (after applicable normalization).
...
- Issue one wallet address per supported currency for the same underlying account. Each wallet address advertises its own asset code and scale.
- Issue a single wallet address in the primary currency and perform currency conversion on the ASE side when payments arrive in a different currency. In this pattern, the wallet address still advertises only one asset code, and the recipient is credited in that currency regardless of the sender's denomination.
...
- Whether disabled wallet addresses are tombstoned (return `410 Gone`) or recycled.
