...
ASEs decide the exact UI, but the consent prompt must accurately reflect what the authorization server will encode in the resulting grant. If the IdP does not accurately reflect the access the ASE received in the grant request, the resource owner may consent to more access than they were shown, or less than the client requested. That exposes both the ASE and the resource owner to risk.
