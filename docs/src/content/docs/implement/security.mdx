...
5. Reject the request if any of the following are true:
   ...
   - The signature is older than the freshness window the ASE permits.
   - The `Content-Digest` header is present but does not match the body.

ASEs decide their freshness window. Open Payments does not bound it, but very long windows create replay risk and very short windows create operational issues for clients on slow networks.
